...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    if "chk_path" in kwargs:
        chk_path = kwargs.get("chk_path")
        kwargs.pop("chk_path", None)
        del default_cfg['url']
        default_cfg['file'] = chk_path
    ...
    model = build_model_with_cfg(
        ...
        #pretrained_custom_load='file' in default_cfg or 'npz' in default_cfg['url'],
        **kwargs)
    return model
...
def passt_hyonj(pretrained=False, **kwargs):
    """ PaSST pre-trained on AudioSet
    """
    print("\n\n Loading PaSST hyonj \n\n")
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    if model_kwargs.get("stride") != (10, 10):
        warnings.warn(
            f"This model was pre-trained with strides {(10, 10)}, but now you set (fstride,tstride) to {model_kwargs.get('stride')}.")
    model = _create_vision_transformer(
        'passt_hyonj', pretrained=pretrained, distilled=True, **model_kwargs)
    return model
...
def get_model(arch="passt_s_swa_p16_128_ap476", pretrained=True, n_classes=527, in_channels=1, fstride=10,
              tstride=10,
              input_fdim=128, input_tdim=998, u_patchout=0, s_patchout_t=0, s_patchout_f=0, chk_path = None
              ):
    """
    ...
    model = model_func(pretrained=pretrained, num_classes=n_classes, in_chans=in_channels,
                       ...
                       s_patchout_t=s_patchout_t, s_patchout_f=s_patchout_f, chk_path=chk_path)
